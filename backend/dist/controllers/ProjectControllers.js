...
exports.getJobseekerProjectsByUserId = exports.deleteUserProject = exports.updateUserProject = exports.addUserProject = exports.getUserProjects = void 0;
...
exports.getUserProjects = (0, asyncHandlers_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const result = await db_config_1.default.query(`SELECT p.*, 
    ...
    res.json((0, helpers_1.formatSuccess)(result.rows, 'User projects retrieved successfully'));
});
// @desc    Add project to user profile
// @route   POST /api/users/projects
// @access  Private (Jobseeker)
exports.addUserProject = (0, asyncHandlers_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const { title, description, start_date, end_date, project_url, image_url, technologies } = req.body;
    // Validate input
    if (!title || !description || !start_date) {
        throw new errorMiddlewares_1.AppError('Title, description and start date are required', 400);
    }
    // Start a transaction
    ...
    try {
        ...
        const projectResult = await client.query(`INSERT INTO user_projects 
       ...
       RETURNING *`, [
        ...
        if (technologies && Array.isArray(technologies) && technologies.length > 0) {
            ...
            await client.query(`INSERT INTO project_technologies (project_id, technology) VALUES ${techValues.join(', ')}`, techParams);
        }
        await client.query('COMMIT');
        // Get complete project with technologies
        const finalResult = await db_config_1.default.query(`SELECT p.*, 
              CASE WHEN COUNT(pt.technology) = 0 THEN '[]'::json
              ELSE json_agg(pt.technology) END AS technologies
       FROM user_projects p
       LEFT JOIN project_technologies pt ON p.id = pt.project_id
       ...
       GROUP BY p.id`, [projectId]);
        res.status(201).json((0, helpers_1.formatSuccess)(finalResult.rows[0], 'Project added to profile successfully'));
    }
    catch (error) {
...
// @access  Private (Jobseeker)
exports.updateUserProject = (0, asyncHandlers_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const projectId = parseInt(req.params.id);
    const { title, description, start_date, end_date, project_url, image_url, technologies } = req.body;
    // Check if project exists and belongs to the user
    const projectExists = await db_config_1.default.query('SELECT * FROM user_projects WHERE id = $1 AND user_id = $2', [projectId, userId]);
    if (projectExists.rows.length === 0) {
        throw new errorMiddlewares_1.AppError('Project not found in your profile or you do not have permission', 404);
    }
    // Start a transaction
    const client = await db_config_1.default.connect();
    try {
      ...
      `;
       ...
       WHERE p.id = $1
       GROUP BY p.id`, [projectId]);
        res.json((0, helpers_1.formatSuccess)(finalResult.rows[0], 'Project updated successfully'));
    }
    catch (error) {
        await client.query('ROLLBACK');
        throw error;
...
// @access  Private (Jobseeker)
exports.deleteUserProject = (0, asyncHandlers_1.default)(async (req, res) => {
    const userId = req.user?.id;
    const projectId = parseInt(req.params.id);
    // Check if project exists and belongs to the user
    const projectExists = await db_config_1.default.query('SELECT * FROM user_projects WHERE id = $1 AND user_id = $2', [projectId, userId]);
    ...
    try {
        ...
        await client.query('COMMIT');
        res.json((0, helpers_1.formatSuccess)(null, 'Project removed from profile successfully'));
    }
    catch (error) {
        await client.query('ROLLBACK');
        throw error;
...
// @access  Public
exports.getJobseekerProjectsByUserId = (0, asyncHandlers_1.default)(async (req, res) => {
    ...
    const result = await db_config_1.default.query(`SELECT p.id, p.title, p.description, p.start_date, p.end_date, p.project_url, p.image_url,
            CASE WHEN COUNT(pt.technology) = 0 THEN '[]'::json
            ELSE json_agg(pt.technology) END AS technologies
     FROM user_projects p
     LEFT JOIN project_technologies pt ON p.id = pt.project_id
     WHERE p.user_id = $1
     GROUP BY p.id
     ORDER BY p.start_date DESC`, [jobseekerId]);
    res.json((0, helpers_1.formatSuccess)(result.rows, 'Jobseeker projects retrieved successfully'));
});
